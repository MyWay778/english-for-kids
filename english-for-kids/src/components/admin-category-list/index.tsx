...
const AdminCategoryList: FC<AdminCategoryListProps> = ({
  categories,
}): ReactElement => {
  const {
  ...
  const { editableCategoryId, isLoading } = useTypedSelector((state) => ({
    ...state.adminPanel,
  }));
      ...
      <CardList>
        {addMode ? (
          <AdminCategoryCard
            category={{
              id: 0,
              name: `Category ${categories.length}`,
