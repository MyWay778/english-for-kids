...
const AdminWordsList: FC<AdminWordsListProps> = ({words}): ReactElement => {
  ...
  const shownWords = useMemo(() => words.slice(0,wordsLimit)
  , [words, wordsLimit]);
  ...
  return (
    <>
      ...
      <CardList>
        {words.length < 8 && (addMode ? <AdminWordsCard saveHandler={saveNewWordHandler}
        ...
        {shownWords.map(word => {
          ...
          return <li key={word.id}><AdminWordsCard  clickEditHandler={clickEditHandler} saveHandler={saveHandler}
                                 deleteHandler={deleteHandler} cancelHandler={clickCancelHandler} word={word}
                                     editable={editable}/></li>
        })}
      </CardList>
